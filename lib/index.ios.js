...
class ReactNativePickerModule extends React.Component {
  ...
  cancelDialog = () => {
    Animated.timing(this.state.animation, {
      toValue: 0,
      duration: this.props.ios.duration
    }).start(() => {
      this.setState({
        isVisible: false
      });
  ...
  componentDidUpdate(prevProps) {
    // if no items left, cancel the dialog
    if (
  ...
  static getDerivedStateFromProps = (props, currentState) => {
    ...
    if (currentState.selectedValue === props.items.length) {
      return {
        selectedValue: currentState.selectedValue - 1
      };
    }
    return null;
  };
        ...
        >
          <Animated.View style={styles.container}>
            ...
            <View style={styles.cancelButton}>
              <TouchableOpacity activeOpacity={0.9} onPress={this.cancelDialog}>
